...
Replace `shortcut_list_shortcuts` with `memo_view_list_memo_views`. Existing saved filters migrate automatically, but client tool names and request mappings must change. Tool calls also follow the new author and Space membership permissions. Comments created without an explicit visibility default to `PRIVATE`. See [API migration notes](/docs/integrations/api-access#031-client-migration).
...
- Public reads may work without a token only when the instance access policy permits public reads and the underlying REST endpoint allows anonymous access. The canonical instance URL does not select that policy.
...
| `memo_view_list_memo_views` | List the current user's saved views |
