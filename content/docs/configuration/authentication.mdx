...
| Personal access token (PAT) | User-selected; no expiration by default | SHA-256 hash in DB | Format: `memos_pat_<32 random chars>` |
...
In 0.31, sign-in and lookup use exact-case usernames. `Alice` and `alice` are distinct spellings. New or renamed usernames must contain 1–36 ASCII letters or digits, with hyphens only inside the name. Existing legacy names remain addressable, but unsupported spellings cannot be used for new mentions. When diagnosing a failed sign-in after upgrading, check the stored username and its case before resetting credentials.
...
For a new SSO user, Memos first tries to use the mapped external identifier as the local username when it satisfies the username rules and is not reserved. If that value is invalid or already belongs to another account, Memos generates a UUID-based username instead. It never links an SSO identity to an existing account merely because the usernames match.
...
- create a separate token for each application or integration
- choose an expiration when the integration does not need a permanent credential; new tokens default to no expiration
- review the last-used timestamp to find abandoned credentials
