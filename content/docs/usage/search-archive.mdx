...
- richer saved filters and saved views
...
Home and Explore filters follow the selected Space. [Saved Views](/docs/usage/saved-views) reuse CEL expressions, including `has_location`, `space == null` for Unassigned memos, and `visibility == "SPACE"` for members-only memos.
